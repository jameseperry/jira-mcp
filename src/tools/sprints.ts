...
export function registerSprintTools(server: McpServer, client: JiraClient) {
  ...
  server.tool(
    ...
    async ({ instance, boardId, state }) => {
      const params = new URLSearchParams();
      if (state) params.set("state", state);
      const data = await client.for(instance).agileGet(`/board/${boardId}/sprint?${params}`);
  ...
  server.tool(
    ...
    async ({ instance, name, boardId, startDate, endDate, goal }) => {
      const body: any = { name, originBoardId: boardId };
      if (startDate) body.startDate = startDate;
      if (endDate) body.endDate = endDate;
      if (goal) body.goal = goal;
      const result = await client.for(instance).agilePost("/sprint", body);
      return {
        content: [
          {
            type: "text",
  ...
  server.tool(
    ...
    async ({ instance, sprintId, maxResults }) => {
      const params = new URLSearchParams();
      if (maxResults) params.set("maxResults", String(maxResults));
      params.set("fields", "summary,status,assignee,issuetype,priority");
      const data = await client.for(instance).agileGet(`/sprint/${sprintId}/issue?${params}`);
  ...
  server.tool(
    ...
    async ({ instance, sprintId, issueKeys }) => {
      await client.for(instance).agilePost(`/sprint/${sprintId}/issue`, {
        issues: issueKeys,
      });
      return {
        content: [
